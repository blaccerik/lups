...
from sqlalchemy import Integer, String, Column, ForeignKey, Enum, Sequence, BigInteger, Boolean
...
uri = f"mysql+pymysql://" \
      ...
      f"{os.environ.get('MYSQL_PASSWORD', 'erik')}@{os.environ.get('MYSQL_BROKER_URL', 'localhost:3306')}/" \
...
class Message(Base):
    ...
    type = Column(Enum("user", "bot"), nullable=False)
    deleted = Column(Boolean, nullable=False, default=False)
