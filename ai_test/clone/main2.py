...
#         return
#     if selected_model == 'None' and False:
...
#             unload_model()
#             if selected_model != '' or True:
#                 print("load_model_wrapper")
...
#
#                 for i in magic(model, tokenizer):
#                     print(i)
#
...
#                         shared.gradio['alpha_value'] = gr.Slider(label='alpha_value', minimum=1, maximum=8, step=1,
#                                                                  info='Positional embeddings alpha factor for NTK RoPE scaling. Same as above. Use either this or compress_pos_emb, not both.',
...
#
#     shared.gradio['loader'].change(loaders.make_loader_params_visible, gradio('loader'),
#                                    gradio(loaders.get_all_params()))
#
...
#         ui.apply_interface_values, gradio('interface_state'), gradio(ui.list_interface_input_elements()),
#         show_progress=False).then(
#         update_model_parameters, gradio('interface_state'), None).then(
#         load_model_wrapper, gradio('model_menu', 'loader', 'autoload_model'), gradio('model_status'),
#         show_progress=False)
#
#     load.click(
#         ui.gather_interface_values, gradio(shared.input_elements), gradio('interface_state')).then(
#         update_model_parameters, gradio('interface_state'), None).then(
#         partial(load_model_wrapper, autoload=True), gradio('model_menu', 'loader'), gradio('model_status'),
#         show_progress=False).then(
...
#
#     shared.gradio['lora_menu_apply'].click(load_lora_wrapper, gradio('lora_menu'), gradio('model_status'),
#                                            show_progress=False)
#     shared.gradio['download_model_button'].click(download_model_wrapper, gradio('custom_model_menu'),
#                                                  gradio('model_status'), show_progress=True)
...
#
#     shared.gradio['preset_menu'].change(presets.load_preset_for_ui, gradio('preset_menu', 'interface_state'),
#                                         gradio('interface_state', 'do_sample', 'temperature', 'top_p', 'typical_p',
...
#             chat.save_character, gradio('name2', 'greeting', 'context', 'character_picture', 'save_character_filename'),
#             None).then(
...
#                 lambda
#                     x: f'session_{shared.get_mode()}_{x + "_" if x not in ["None", None, ""] else ""}{utils.current_time()}.json',
#                 gradio('character_menu'), gradio('save_filename')).then(
#                 lambda: gr.update(visible=True), None, gradio('file_saver'))
#
...
#                     value=chat_html_wrapper({'internal': [], 'visible': []}, shared.settings['name1'],
...
#                             with gr.Row():
#                                 shared.gradio['character_menu'] = gr.Dropdown(value='None',
...
#                                                                     step=1, label='max_new_tokens',
#                                                                     value=shared.settings['max_new_tokens'])
#                         with gr.Row():
#                             shared.gradio['prompt_menu'] = gr.Dropdown(choices=utils.get_available_prompts(),
#                                                                        value='None', label='Prompt',
#                                                                        elem_classes='slim-dropdown')
#                             ui.create_refresh_button(shared.gradio['prompt_menu'], lambda: None,
...
#                 with gr.Row():
#                     with gr.Column():
#                         shared.gradio['textbox'] = gr.Textbox(value=default_text, elem_classes="textbox_default",
#                                                               lines=27, label='Input')
#                         shared.gradio['max_new_tokens'] = gr.Slider(minimum=shared.settings['max_new_tokens_min'],
#                                                                     maximum=shared.settings['max_new_tokens_max'],
#                                                                     step=1, label='max_new_tokens',
#                                                                     value=shared.settings['max_new_tokens'])
...
#                     with gr.Column():
#                         with gr.Tab('Raw'):
#                             shared.gradio['output_textbox'] = gr.Textbox(elem_classes="textbox_default_output",
#                                                                          lines=27, label='Output')
#
#                         with gr.Tab('Markdown'):
#                             shared.gradio['markdown_render'] = gr.Button('Render')
#                             shared.gradio['markdown'] = gr.Markdown()
...
#                 with gr.Column():
#                     with gr.Row():
#                         shared.gradio['interface_modes_menu'] = gr.Dropdown(choices=modes, value=current_mode,
#                                                                             label="Mode", elem_classes='slim-dropdown')
#                         shared.gradio['reset_interface'] = gr.Button("Apply and restart", elem_classes="small-button",
#                                                                      variant="primary")
...
#                     if not shared.args.multi_user:
#                         with gr.Row():
#                             shared.gradio['session_menu'] = gr.Dropdown(choices=utils.get_available_sessions(),
...
#                             ui.create_refresh_button(shared.gradio['session_menu'], lambda: None,
...
#                 lambda: gr.update(choices=utils.get_available_extensions(), value=shared.args.extensions), None,
...
#
#             shared.gradio['toggle_dark_mode'].click(lambda: None, None, None,
...
#                 chat.generate_chat_reply_wrapper, shared.input_params, gradio('display', 'history'),
#                 show_progress=False).then(
#                 ui.gather_interface_values, gradio(shared.input_elements), gradio('interface_state')).then(
...
#                 ui.gather_interface_values, gradio(shared.input_elements), gradio('interface_state')).then(
#                 partial(chat.generate_chat_reply_wrapper, regenerate=True), shared.input_params,
#                 gradio('display', 'history'), show_progress=False).then(
#                 ui.gather_interface_values, gradio(shared.input_elements), gradio('interface_state')).then(
#                 chat.save_persistent_history, gradio('history', 'character_menu', 'mode'), None).then(
#                 lambda: None, None, None, _js=f"() => {{{audio_notification_js}}}")
#             )
...
#                 partial(chat.load_character, instruct=False), gradio('character_menu', 'name1', 'name2'),
#                 gradio('name1', 'name2', 'character_picture', 'greeting', 'context', 'dummy')).then(
#                 ui.gather_interface_values, gradio(shared.input_elements), gradio('interface_state')).then(
...
#                 lambda x: gr.update(visible=x != 'instruct'), gradio('mode'), gradio('chat_style'),
#                 show_progress=False).then(
#                 chat.redraw_html, shared.reload_inputs, gradio('display'))
#
...
#                 gradio('name1_instruct', 'name2_instruct', 'dummy', 'dummy', 'context_instruct', 'turn_template'))
#
#             shared.gradio['upload_chat_history'].upload(
#                 chat.load_history, gradio('upload_chat_history', 'history'), gradio('history')).then(
#                 chat.redraw_html, shared.reload_inputs, gradio('display'))
#
#             shared.gradio['Copy last reply'].click(chat.send_last_reply_to_input, gradio('history'), gradio('textbox'),
#                                                    show_progress=False)
#
...
#             shared.gradio['Submit character'].click(chat.upload_character, gradio('upload_json', 'upload_img_bot'),
#                                                     gradio('character_menu'))
#             shared.gradio['upload_json'].upload(lambda: gr.update(interactive=True), None, gradio('Submit character'))
#             shared.gradio['upload_json'].clear(lambda: gr.update(interactive=False), None, gradio('Submit character'))
#
#             shared.gradio['Submit tavern character'].click(chat.upload_tavern_character,
#                                                            gradio('upload_img_tavern', 'tavern_json'),
#                                                            gradio('character_menu'))
#             shared.gradio['upload_img_tavern'].upload(chat.check_tavern_character, gradio('upload_img_tavern'),
#                                                       gradio('tavern_name', 'tavern_desc', 'tavern_json',
#                                                              'Submit tavern character'), show_progress=False)
#             shared.gradio['upload_img_tavern'].clear(lambda: (None, None, None, gr.update(interactive=False)), None,
#                                                      gradio('tavern_name', 'tavern_desc', 'tavern_json',
#                                                             'Submit tavern character'), show_progress=False)
...
#                 )
#             else:
#                 shared.gradio['markdown_render'].click(lambda x: x, gradio('output_textbox'), gradio('markdown'),
#                                                        queue=False)
#                 gen_events.append(shared.gradio['Continue'].click(
#                     ui.gather_interface_values, gradio(shared.input_elements), gradio('interface_state')).then(
#                     generate_reply_wrapper, [shared.gradio['output_textbox']] + shared.input_params[1:], output_params,
#                     show_progress=False).then(
#                     ui.gather_interface_values, gradio(shared.input_elements), gradio('interface_state')).then(
#                     lambda: None, None, None, _js=f"() => {{{audio_notification_js}}}")
...
#                 )
#
#             shared.gradio['Stop'].click(stop_everything_event, None, None, queue=False,
#                                         cancels=gen_events if shared.args.no_stream else None)
#             shared.gradio['prompt_menu'].change(load_prompt, gradio('prompt_menu'), gradio('textbox'),
#                                                 show_progress=False)
...
#                                         gradio(ui.list_interface_input_elements()), show_progress=False)
